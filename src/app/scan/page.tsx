...
import MovieSearchSkeleton from "@/components/MovieSearchSkeleton";
...
function ScanPageInner() {
  ...
  // useMovieSearch handles debounce (300ms), localStorage cache, and stale results
  const { results: rawResults, loading } = useMovieSearch(query);
  const results = rawResults.slice(0, 8).map(toScanMovie);
      ...
      <div className="mx-auto min-h-[calc(100dvh-2.5rem)] w-full max-w-5xl pb-28">
        ...
        <div className="mt-8 border-t border-white/10">
          {loading && results.length === 0 ? (
            ...
            <MovieSearchSkeleton count={5} />
          ) : loading && results.length > 0 ? (
            // Subsequent load — show stale results with a subtle shimmer overlay
            <div className="relative">
              <div className="pointer-events-none absolute inset-0 z-10 animate-pulse rounded-xl bg-white/[0.02]" />
              <div className="max-h-[34rem] overflow-y-auto opacity-60">
                {results.map((movie) => (
                  <MovieRow key={movie.id} movie={movie} onSelect={handleSelectMovie} />
                ))}
              </div>
            </div>
          ) : results.length > 0 ? (
            ...
            <div className="flex min-h-[14rem] flex-col items-start justify-center py-8">
              <Film className="h-10 w-10 text-white/20" />
              <p className="mt-4 text-base font-semibold text-[#f5f0de]">No results found</p>
              <p className="mt-1 text-sm text-white/40">Try a different title or year</p>
            </div>
          ) : (
...
function MovieRow({ movie, onSelect }: { movie: ScanMovie; onSelect: (m: ScanMovie) => void }) {
  return (
    <button
      type="button"
